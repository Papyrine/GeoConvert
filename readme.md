...
**[Try it in the browser →](https://geoconvert.papyrine.org/)** — a Blazor WebAssembly app that converts maps entirely client-side (no data leaves the local device).
...
When rendering the same data at different canvas sizes — or the same canvas at different bounding boxes — a fixed pixel stroke width is too thick for a small render and too thin for a large one. Worse, at a small canvas a dense map (thousands of tiny polygons) collapses into a solid mass as every border lands on the same handful of pixels. `RenderOptions.StrokeAutoScale` (on by default) solves this with a zoom-aware multiplier: the implicit zoom is derived from the canvas/bbox ratio, and `StrokeWidth` / `PointRadius` are scaled by `√2^(zoom − 10)` clamped to `[0.1, 6]` — the stroke halves for every two zoom levels below the country-scale (zoom ~10) anchor and doubles for every two above, keeping the stroke roughly proportional to the output's pixel density. A thumbnail or world view thins right down; the thinned-down sub-pixel strokes render as faint antialiased hairlines (the rasterizer fades a sub-1px line's alpha by its width rather than clamping it to a solid 1px band), so a dense map reads as a legible density gradient instead of black. Label size is intentionally not scaled, matching what every shipping web map does. Set `StrokeAutoScale = false` for a fixed pixel `StrokeWidth` regardless of scale.
