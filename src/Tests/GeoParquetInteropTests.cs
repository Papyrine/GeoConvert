...
public class GeoParquetInteropTests
{
    ...
    public async Task Reads_parquet_net_dictionary_and_snappy()
    {
        ...
        await Assert.That(((Point) features[0].Geometry!).Coordinate.X).IsEqualTo(1d);
        await Assert.That(((Point) features[1].Geometry!).Coordinate.X).IsEqualTo(3d);
        await Assert.That(features[0].Properties["name"]).IsEqualTo("town");
        await Assert.That(features[0].Properties["pop"]).IsEqualTo(5L);
        await Assert.That(features[1].Properties.ContainsKey("pop")).IsFalse();
        // INT32 physical column surfaces as long.
        await Assert.That(features[0].Properties["count"]).IsEqualTo(7L);
    }
}
